...
def ensure_remote_agent(host: str, user: str, key_path: str, update: bool = False):
    ...
    try:
        ...
        if update:
            print("Updating remote_server.py on remote VM...")
            local_path = Path(__file__).parent.parent / "remote_server.py"
            sftp.put(local_path.as_posix(), f"/Users/{user}/remote_server.py")
            print("Restarting agent due to update...")
            ssh.exec_command(f"pkill -f /Users/{user}/remote_server.py || true")
        else:
            try:
...
<dict>
  ...
  <key>StandardErrorPath</key> <string>/tmp/agent.err</string>
        ...
        if http_code == "200":
            ...
            ssh.exec_command(
                f"nohup {venv_path}/bin/python3 /Users/{user}/remote_server.py >/tmp/agent.out 2>&1 &"
            )
            time.sleep(3)
            stdin, stdout, stderr = ssh.exec_command(
                "curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:9000/health || true"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--host",
        required=True,
        help="VM hostname or IP address"
    )
    parser.add_argument(
        "--user",
        required=True,
    ...
    parser.add_argument(
        ...
        "--update",
        action="store_true",
        help="Force update of remote_server.py on remote VM",
    )
    args = parser.parse_args()
    ensure_remote_agent(args.host, args.user, args.key_path, update=args.update)
